...
public:
    ...
    MinHeap(bool pt = false) {
        are_pointer_types = pt;
        init(MAX_HEAP_MAX_SIZE);
    }


    MinHeap(uint32_t size, bool pt = false) {
        are_pointer_types = pt;
        init(size);
    }
...
template<typename T>
std::ostream& operator << (std::ostream& o, MinHeap<T>* m) {
    std::string t;
    m->ToString(t);
    return o << t;
}
...
HuffmanTreeNode* GetHuffmanTree(uint32_t* freqs) {
    ...
    auto m = new MinHeap<HuffmanTreeNode*>();
    ...
    while (m->Size() != 1) {
        ...
        m->Insert(new HuffmanTreeNode('S', sum, left, right)); //S is a special character O(logn)
    }//logn
    return  m->Pop();
}
...
std::string HuffmanCompress(const char* str, HuffmanTreeNode& r) {
    ...
    auto freqs = GetFreqsOfChars(str); //O(n)
    //PrintFrequencyOfChars(freqs);
    //Looks good!

    auto tree = GetHuffmanTree(freqs); //O(nlogn)hu
    r = *tree;
    //m->Pop();
    ...
    BuildHuffmanCode(tree, codes, tmp); //used to compress files O(1)
    std::cout << "\nCodes:\n";
    for (int i = 0; i < 128; i++) {
        if (codes[i].size()) {
...
int main() {
    ...
    if (original.size() <= 1) {
        cerr << "Canno compress this length!\a\n";
        exit(EXIT_FAILURE);
    }
    HuffmanTreeNode root;
    auto compressed = HuffmanCompress(original.c_str(), root);
